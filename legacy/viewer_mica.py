from keyframe_extractor_mica import extract_keyframes
...
def visualize_keyframes(input_video, speed=1):
    return extract_keyframes(input_video)

def similarity_f1(predicted, desired, tolerance=3):
    matched_pred = set()
    matched_desired = set()

    for i, p in enumerate(predicted):
        for j, d in enumerate(desired):
            if abs(p - d) <= tolerance:
                matched_pred.add(i)
                matched_desired.add(j)

    true_positives = len(matched_pred)
    precision = true_positives / len(predicted) if predicted else 0
    recall = true_positives / len(desired) if desired else 0
...
list = [
    [20,100,140,200,250],
    [20,80,100,140,200,250],
    [20,100,140,200,260],
    [30,120,150,230],
    [30,65,100,180,240,260],
    [50,70,100,170,220,260]
]
if __name__ == '__main__':
    for i in [1]:
        ...
        for video_path in video_files:
            ...
            normalized_scores, keyframes = visualize_keyframes(video_path)
            print(f"[Video] {name}\nKeyframes: {keyframes}\n")
            save_keyframes(video_path, keyframes, output_dir=f"vivo{i}")
            #name2 = str(similarity_f1(predicted=keyframes,desired=list[i-1],tolerance=int(0.04*len(normalized_scores))))

            #Show_comparison(normalized_scores,keyframes, name,behaviorN=i,desired_keyframes=list[i-1],name2=name2)
